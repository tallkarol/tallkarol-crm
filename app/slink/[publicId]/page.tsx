      ...
      <header className="border-b border-line bg-tk-white px-5 py-6">
        <div className="mx-auto grid max-w-3xl gap-2">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-[11.5px]">
            ...
            <Badge tone={recipient.expiresAt ? "warn" : "ok"}>{timeLeftLabel(recipient.expiresAt, now)}</Badge>
          </div>
        </div>
      </header>

      <div className="mx-auto grid max-w-3xl gap-4 px-5 py-6">
        {slink.intro ? <p className="max-w-[68ch] text-[13.5px] leading-relaxed text-tk-slate">{slink.intro}</p> : null}
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-line bg-tk-white px-4 py-3 text-[12.5px] text-tk-slate">
          <span className="flex-1">
            <b className="text-tk-onyx">This link is personal to you.</b> Forwarding it will not work, because it is
...
function ResendForm({ publicId, email, label }: { publicId: string; email?: string; label: string }) {
  return (
...
function Locked({ publicId, title, reason }: { publicId: string; title: string; reason: string }) {
  return (
    <main className="grid min-h-screen place-items-center bg-well px-5 py-10">
      <div className="w-full max-w-md rounded-xl border border-line bg-tk-white p-6">
        ...
        <p className="mt-2 text-[13px] text-tk-slate">
          Enter the address this was shared with and a new link is on its way. Nothing behind it has changed or been
          deleted.
        </p>
        <div className="mt-4">
