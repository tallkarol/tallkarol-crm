...
export function LeftOffBoard({
  ...
  const selection = useMemo((): Selection => {
    ...
    if (item) {
      // A chat reaches Morning as a `blocked_chat` queue row, whose detail is
      // a subtitle and a date. The board is already holding the whole
      // conversation — hand the drawer that instead. It is the difference
      // between an "Open" link and Resume, the reply box and the handoff.
      if (item.kind === "blocked_chat") {
        const ref = item.id.slice(item.id.indexOf(":") + 1)
        const chat = notes.find((n) => n.sessionRef === ref)
        if (chat) return { kind: "note", note: chat }
      }
  ...
  useEffect(() => {
    ...
    /** Fire a control in the drawer by its `data-act` name. */
    const act = (name: string) => {
      const node = document.querySelector<HTMLElement>(`aside[aria-label="Details"] [data-act="${name}"]`)
    ...
    function onKey(event: KeyboardEvent) {
      ...
      if (event.key === "Escape") {
        ...
        // The drawer is a layer, and on a phone it covers the list entirely.
        // Esc has to take it off before it takes the board off.
        if (selected) {
          setSelected(null)
          return
        }
      ...
      switch (event.key) {
        ...
        case "ArrowRight":
          ...
          return laneStep(1)
        case "Enter": {
          // Nothing left to "open" — the drawer is already showing it. Enter
          // hands the keyboard to the drawer's own controls instead.
          if (!selected) return
          event.preventDefault()
          const first = document.querySelector<HTMLElement>(
            'aside[aria-label="Details"] a, aside[aria-label="Details"] button'
          )
          first?.focus()
          return
        }
  ...
  return (
    <div className="fixed inset-0 z-[70]">
      ...
      >
        ...
        {/* ---------------------------------------------------------- body */}
        {/*
          Morning is a two-pane and stays one whether or not something is
          selected: the list wants a measure — a 1,600px row was the whole
          complaint — and a column that resizes when you click is worse than
        ...
        {selection ? (
          ...
          >
            <p className="max-w-[26ch] px-6 text-center text-[12.5px] leading-relaxed text-ink-3">
              ...
              — and everything about it opens here.
            </p>
          </aside>
        ) : null}
        </div>
...
function NoteCard({
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "line-clamp-2 text-[13px] font-semibold leading-snug tracking-tight text-tk-onyx [overflow-wrap:anywhere]",
          rail && "text-[11.5px]"
        )}
      >
        {note.title}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "bg-well" : "bg-card hover:bg-well"
      )}
    >
      <span
        aria-hidden
...
const DRAWER_BODY = "grid w-full max-w-[68ch] content-start gap-4 overflow-y-auto px-5 py-4"
