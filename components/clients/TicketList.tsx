...
export function TicketList({ tickets }: { tickets: HubTicket[] }) {
  if (tickets.length === 0) {
    return <p className="px-5 py-8 text-sm text-tk-slate/60">No open tickets.</p>
  }
  return (
    <div className="divide-y divide-tk-slate/10">
      {tickets.map((t) => (
        <div key={t.id} className="px-5 py-4">
          ...
          {t.snippet ? (
            <p className="mt-2 border-l-2 border-tk-slate/15 pl-2.5 text-[13px] leading-snug text-tk-slate/70">
              “{t.snippet}
              {t.snippet.length >= 200 ? "…" : ""}”
            </p>
          ) : null}
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-tk-slate/50">
            <span>
