    ...
    <section className="overflow-hidden rounded-2xl border border-tk-slate/15 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-tk-slate/10 px-5 py-3.5">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-tk-slate/60">{note}</p>
        </div>
        <div className="flex rounded-lg border border-tk-slate/20 bg-tk-linen/70 p-0.5">
          {VIEWS.map((option) => (
            <button
              key={option.id}
              type="button"
      ...
      {view === "billed" ? (
        ...
        <p className="px-5 pb-4 pt-2 text-xs text-tk-slate/60">
