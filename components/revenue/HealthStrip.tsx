...
function Cell({
  ...
  return (
    <div className="border-l border-t border-tk-slate/10 px-4 py-3.5 first:border-l-0 sm:border-t-0">
      <p className="text-[10px] font-bold uppercase tracking-widest text-tk-slate/60">
        {label}
      </p>
      <p
      ...
      >
        {value}
        {unit ? (
          <span className="text-xs font-medium text-tk-slate/60">{unit}</span>
        ) : null}
      </p>
      <p className="mt-0.5 text-[11px] tabular-nums text-tk-slate/60">{sub}</p>
    </div>
  )
}
