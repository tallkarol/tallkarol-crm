...
export function SiteUptimeCard({
  ...
  return (
    <Link
      href={peekHref}
      scroll={false}
      className="group flex overflow-hidden rounded-xl border border-tk-slate/15 bg-white shadow-sm transition-colors hover:border-tk-slate/30"
    >
      <span aria-hidden className="w-[3px] shrink-0" style={{ background: color }} />
      <div className="min-w-0 flex-1 px-3 py-2">
        <p className="flex items-center justify-between gap-2">
          <span className="truncate text-[10px] font-semibold uppercase tracking-wide text-tk-slate/50">
          ...
          <span
            ...
            className={cn(
              ...
              bad ? "bg-[#B4322A]" : idle ? "bg-tk-slate/25" : "bg-[#2E7D57]"
            )}
          />
        </p>
        ...
        <p className="truncate font-mono text-[10.5px] text-tk-slate/45">
          {hostnameOf(monitor?.url || site.origin) || site.slug}
        </p>
        <p
          className={cn(
            "mt-1 flex items-baseline gap-1.5 font-mono text-[10.5px]",
            bad ? "font-semibold text-[#B4322A]" : "text-tk-slate/50"
          )}
        >
          ...
          {last ? (
            <>
              <span className="text-tk-slate/25">·</span>
              <span className="text-tk-slate/35">{ageLabel(last.at)} ago</span>
            </>
          ) : null}
        </p>
        <span className="mt-1.5 flex h-3.5 items-end gap-px" aria-hidden>
          {pings.length === 0
            ? Array.from({ length: STRIP }, (_, i) => (
                <span key={i} className="min-w-0 flex-1 rounded-[1px] bg-tk-slate/10" style={{ height: "20%" }} />
              ))
            : pings.map((ping) => (
                <span
