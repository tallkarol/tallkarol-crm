      ...
      {detail ? (
        <>
          <Link
            ...
            className="fixed inset-0 z-[60] bg-scrim backdrop-blur-[2px] xl:hidden"
          />
          <aside className="fixed inset-y-0 right-0 z-[70] flex w-full max-w-[32rem] flex-col bg-card shadow-overlay motion-safe:animate-[tk-peek-in_.2s_ease-out] xl:static xl:z-auto xl:w-[27.5rem] xl:max-w-none xl:shrink-0 xl:animate-none xl:border-l xl:border-line xl:shadow-none">
            {detail}
          </aside>
        </>
      ) : (
        <aside className="hidden w-[27.5rem] shrink-0 flex-col items-center justify-center border-l border-line px-8 text-center xl:flex">
          <h2 className="text-sm font-semibold text-ink-3">Pick a ticket</h2>
          <p className="mt-1.5 max-w-[26ch] text-[13px] text-ink-3">
            It opens at <code className="rounded bg-well px-1 py-0.5 text-[11px]">/support/&lt;number&gt;</code>{" "}
    ...
    <div className="flex items-center gap-2 text-[12px] text-ink-3">
      <dt className="flex gap-1">
        {keys.map((k) => (
          <kbd
            key={k}
            className="min-w-5 rounded border border-line bg-well px-1.5 text-center font-mono text-[10.5px] text-tk-slate"
          >
            {k}
          </kbd>
        ))}
