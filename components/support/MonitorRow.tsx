    ...
    <div className="flex border-b border-line last:border-0">
      ...
      <div className="min-w-0 flex-1 px-3.5 py-2.5">
        ...
        <p className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11.5px] text-ink-3">
          ...
          {monitor.paused ? (
            <>
              <span className="text-ink-3">·</span>
              <span className="rounded bg-well px-1.5 font-mono text-[10px]">paused</span>
            </>
          ) : null}
          {ticket ? (
            <>
          ...
          {overdue && !failing ? (
            <>
              <span className="text-ink-3">·</span>
              <span className="font-semibold text-warn">window open</span>
            </>
          ) : null}
        </p>
      </div>
      <div className="flex shrink-0 items-center gap-3 px-3.5 py-2.5">
        <span className="hidden items-end gap-[2px] sm:flex" aria-hidden>
          {runs.length === 0 ? (
            <span className="font-mono text-[11px] text-ink-3">no runs yet</span>
          ) : (
            runs.map((run) => (
              <span
                key={run.id}
        ...
        <span
          className={cn(
            ...
            failing ? "font-semibold text-bad" : "text-ink-3"
          )}
        >
          {last
...
function beatTone(status: string) {
  if (status === "succeeded") return "bg-good"
  if (status === "partial") return "bg-warn"
  if (status === "running") return "bg-tk-teal/50"
  if (status === "missed") return "bg-line-strong"
  return "bg-bad"
}
