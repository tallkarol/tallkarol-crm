    ...
    <>
            ...
            {upcoming.map((lead) => (
              <li key={lead.id}>
                <button
                  ...
                  className="rounded-xl border border-line px-3 py-2 text-left hover:border-line-strong hover:-translate-y-px transition-[transform,box-shadow,border-color,color] duration-150 motion-reduce:transition-none motion-reduce:hover:translate-y-0"
                >
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,22rem)_minmax(0,1fr)]">
        <section className="overflow-hidden rounded-2xl border border-line bg-card shadow-card">
          ...
          {visible.length === 0 ? (
            <p className="px-4 py-10 text-sm text-ink-3">
                  ...
                  <li key={lead.id}>
                    <button
                      ...
                      className={cn(
                        ...
                        active ? "bg-tk-teal/10" : "hover:bg-well"
                      )}
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="shrink-0 rounded-full bg-well px-2 py-0.5 text-[11px] font-semibold text-tk-slate">
              ...
              return (
                <button
                  ...
                  className={
                    active
                      ? "rounded-full bg-accent px-3 py-1.5 text-xs font-semibold text-tk-linen"
                      : "rounded-full border border-line px-3 py-1.5 text-xs font-semibold text-tk-slate hover:border-line-strong hover:-translate-y-px transition-[transform,box-shadow,border-color,color] duration-150 motion-reduce:transition-none motion-reduce:hover:translate-y-0 hover:text-tk-teal disabled:opacity-50"
                  }
                >
          ...
          <dl className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            {lead.formLines.map((line) => (
              <div key={`${line.label}-${line.value}`}>
                <dt className="text-xs text-ink-3">{line.label}</dt>
      ...
      <section className="rounded-2xl border border-line bg-card p-5 shadow-card">
        ...
        >
          <label className="block text-sm">
            <span className="text-xs font-medium text-ink-3">When</span>
            <input
              name="meetingAt"
              type="datetime-local"
          ...
          <label className="block text-sm">
            <span className="text-xs font-medium text-ink-3">Notes</span>
            <input
              name="meetingNotes"
