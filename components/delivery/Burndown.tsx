      ...
      <Card radius="xl" elevation="none" className="relative flex h-[104px] items-end gap-2 px-3 pb-0 pt-3.5">
        {cap > 0 ? (
          <>
            <span
              aria-hidden
              className="absolute inset-x-3 border-t border-dashed border-transparent"
              style={{ bottom: `calc(18px + ${(cap / ceiling) * 68}px)` }}
            />
            <span className="absolute right-3 top-0.5 font-mono text-[9px] text-bad">
              {cap}h ceiling
            </span>
          </>
        ) : null}
          ...
          <div key={m.key} className="flex h-full flex-1 flex-col items-center justify-end gap-1">
            ...
            <span
              ...
              className={cn(
                ...
                m.current ? "bg-accent" : m.billed ? "bg-tk-teal/25" : "bg-tk-teal/40",
                m.hours > cap && cap > 0 && "bg-bad"
              )}
