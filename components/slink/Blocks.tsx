...
export function BlockShell({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-line bg-tk-white">
      ...
      {note ? <p className="border-b border-line px-4 py-2.5 text-[13px] text-ink-3">{note}</p> : null}
...
export function TextBlock({ data }: { data: unknown }) {
  const body = typeof (data as { body?: unknown })?.body === "string" ? (data as { body: string }).body : ""
  return (
...
export function FileBlock({
  ...
  return (
    <ul className="grid gap-2">
      {files.map((f) => (
        <li key={f.id} className="flex items-center gap-3 rounded-lg border border-line bg-well px-3 py-2.5">
          <span className="min-w-0 flex-1 truncate text-[13px] text-tk-onyx">{f.name}</span>
          <span className="font-mono text-[11.5px] tabular-nums text-ink-3">{formatBytes(f.bytes)}</span>
          <a
  ...
  return (
    <div className="grid">
      {list.items.map((item, i) => (
        ...
        >
          <span className="font-mono text-[11px] tabular-nums text-ink-3">{String(i + 1).padStart(2, "0")}</span>
          <span className="text-[13px] text-tk-onyx">
            <span className={item.status === "done" ? "text-ink-3 line-through" : ""}>{item.title}</span>
            {item.outcome ? <small className="mt-0.5 block text-[11.5px] text-ink-3">{item.outcome}</small> : null}
          </span>
          <span
...
export function ReportsBlock({ reports }: { reports: ReportView[] }) {
  ...
  return (
    <ul className="grid gap-2">
      {reports.map((r) => (
        <li key={r.id} className="flex items-center gap-3 rounded-lg border border-line bg-well px-3 py-2.5">
          <span className="min-w-0 flex-1 text-[13px] text-tk-onyx">
            {r.title}
            {r.periodLabel ? <small className="mt-0.5 block text-[11.5px] text-ink-3">{r.periodLabel}</small> : null}
          </span>
          <Link
            href={r.href}
            className="rounded-md border border-line px-2.5 py-1 font-['Inter_Tight',sans-serif] text-[11.5px] font-semibold text-tk-slate hover:border-line-strong hover:-translate-y-px transition-[transform,box-shadow,border-color,color] duration-150 motion-reduce:transition-none motion-reduce:hover:translate-y-0 hover:text-tk-teal"
